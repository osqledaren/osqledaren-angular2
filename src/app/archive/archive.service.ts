...
export class ArchiveService extends ContentService {
  ...
  protected map(res: Response) {
    ...
    const dist = new Dictionary<number, number[]>();
  ...
  private applyDateFilter(year: number, month?: number) {
    ...
    const lastYear: number = years.length - 1;
    let dateString = '';
    ...
    if (year >= years[0] &&
      ...
      if (!isNullOrUndefined(month)) {
        const lastMonth = this._distributions.getValue(year).length - 1;
        ...
        if (!isNullOrUndefined(this._filter.searchTerm)) {
          ...
          if (this._filter.searchTerm.length > 0) {
            ...
            if (this._filter.dateString.length > 0) {
              ...
              const routerParams: Array<string> = ['/nyheter', 'arkiv'];
              routerParams.push(this._filter.dateString);
              routerParams.push(this._filter.searchTerm);

              return this.router.navigate(routerParams);
            } else {
              return this.router.navigate(['/nyheter', 'sok', this._filter.searchTerm]);
            }
          } else if (this._filter.dateString.length > 0) {
            return  this.router.navigate(['/nyheter', 'arkiv', this._filter.dateString]);
          } else {
            return this.router.navigate(['/nyheter']);
          }
        }
